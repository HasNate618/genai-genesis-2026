...
def _new_job(goal: str, coder_count: int) -> dict[str, Any]:
    return {
        ...
        "agent_results": _initial_agent_results(),
        "output_path": None,
        "written_files": [],
...
def _logical_agent_states(job: dict[str, Any]) -> dict[str, str]:
    states = job.get("agent_states") or {}
    return {
        "planner": states.get("planner", "idle"),
        "task_coordinator": states.get("task_coordinator", "idle"),
        "conflict_analyst": states.get("conflict_analyst", "idle"),
        "user_agents": states.get("user_agents", "idle"),
        "merge_agent": states.get("merge_agent", "idle"),
        "qa_agent": states.get("qa_agent", "idle"),
    }


def _initial_agent_results() -> dict[str, str]:
    return {
        "planner": "",
        "task_coordinator": "",
        "conflict_analyst": "",
...
def _set_agent_result(job: dict[str, Any], logical_key: str, content: str) -> None:
    results = job.get("agent_results")
    if not isinstance(results, dict):
        job["agent_results"] = _initial_agent_results()
        results = job["agent_results"]
    results[logical_key] = content
...
def _safe_output_relative_path(path_value: Any, *, fallback: str) -> Path:
    ...
    if not safe_parts:
        safe_parts = [part for part in PurePosixPath(fallback).parts if part not in ("", ".", "..")]
    if not safe_parts:
        safe_parts = ["generated_artifact.txt"]
...
def _simulated_file_content(
    *,
    ...
    goal_line = _truncate_text(goal, limit=260)
    plan_line = _truncate_text(plan, limit=320)
    task_line = ", ".join(task_ids) if task_ids else "none"
    header = (
        '"""\n'
        f"Simulated code artifact generated by {agent_id}.\n"
        f"Execution round: {execution_round}\n"
        f"Prompt goal: {goal_line}\n"
        f"Assigned tasks: {task_line}\n"
        '"""\n\n'
    )
    body = (
        f"# Target file: {changed_file}\n"
        f"# Patch summary: {patch_summary}\n\n"
        f"PROMPT_GOAL = {goal_line!r}\n"
        f"PATCH_SUMMARY = {patch_summary!r}\n"
        f"TASK_IDS = {task_ids!r}\n"
        f"PLAN_REFERENCE = {plan_line!r}\n\n"
        "def generated_stub() -> dict[str, str | list[str]]:\n"
        "    return {\n"
        "        'goal': PROMPT_GOAL,\n"
...
def _build_round_review_markdown(
    *,
    execution_round: int,
    goal: str,
    plan: str,
    ...
    assignments = (task_distribution or {}).get("assignments")
    ...
    for assignment in assignments:
        if not isinstance(assignment, dict):
            continue
        task_id = _truncate_text(assignment.get("task_id"), limit=60)
        summary = _truncate_text(assignment.get("task_summary"), limit=140)
        agent = _truncate_text(assignment.get("assigned_agent_id"), limit=40)
        assignment_lines.append(f"- `{task_id}` -> **{agent}**: {summary}")
    if not assignment_lines:
        assignment_lines = ["- No explicit task assignment records were available."]

    output_lines = []
    for output in outputs:
        if not isinstance(output, dict):
            continue
        agent_id = _truncate_text(output.get("agent_id"), limit=40)
        patch_summary = _truncate_text(output.get("patch_summary"), limit=180)
        files = _string_list(output.get("changed_files"))
        files_text = ", ".join(files) if files else "none"
        task_ids = _string_list(output.get("task_ids"))
        task_text = ", ".join(task_ids) if task_ids else "none"
        output_lines.append(
            f"- **{agent_id}** | tasks: `{task_text}` | files: `{files_text}` | summary: {patch_summary}"
        )
    if not output_lines:
        output_lines = ["- No user-agent outputs were captured."]

    merge_summary = (merge_result or {}).get("summary") if isinstance(merge_result, dict) else {}
    qa_summary = (qa_result or {}).get("summary") if isinstance(qa_result, dict) else {}
    qa_failure_report = (qa_result or {}).get("failure_report") if isinstance(qa_result, dict) else {}
    if not isinstance(merge_summary, dict):
        merge_summary = {}
    if not isinstance(qa_summary, dict):
        qa_summary = {}
    if not isinstance(qa_failure_report, dict):
        qa_failure_report = {}
    root_causes = _string_list(qa_failure_report.get("root_causes"))

    goal_keywords = sorted(
        {
            token.lower()
            for token in re.findall(r"[A-Za-z0-9_]{4,}", goal or "")
            if token.lower() not in {"with", "from", "that", "this", "into", "your", "have"}
        }
    )[:10]
    evidence_blob = " ".join(
        [
    ...
    for keyword in goal_keywords:
        coverage_lines.append(f"- `{keyword}`: {'found' if keyword in evidence_blob else 'missing'}")
    if not coverage_lines:
        coverage_lines = ["- No meaningful keywords were detected in the prompt goal."]

    coordinator_note = _truncate_text((coordinator_context or {}).get("reason"), limit=200)
    execution_note = _truncate_text((execution_context or {}).get("reason"), limit=200)

    return "\n".join(
        [
            f"# Round {execution_round} Review Report",
            "",
            "## Prompt Goal",
            f"- {_truncate_text(goal, limit=500)}",
            "",
            "## Plan Snapshot",
            f"- {_truncate_text(plan, limit=600)}",
            "",
            "## Task Assignment Review",
            *assignment_lines,
            "",
            "## Implementation Output Review",
            *output_lines,
            "",
            "## Merge Gate Review",
            f"- Status: `{(merge_result or {}).get('status', 'unknown')}`",
            f"- Mergeable: `{(merge_result or {}).get('mergeable', False)}`",
            f"- Files touched: `{_safe_int(merge_summary.get('files_touched'), 0)}`",
            f"- Conflicts detected: `{_safe_int(merge_summary.get('conflicts_detected'), 0)}`",
            f"- Next action reason: {_truncate_text((merge_result or {}).get('next_action_reason'), limit=240)}",
            "",
            "## QA Gate Review",
            f"- Status: `{(qa_result or {}).get('status', 'unknown')}`",
            f"- QA passed: `{(qa_result or {}).get('qa_passed', False)}`",
            f"- Commands run: `{_safe_int(qa_summary.get('commands_run'), 0)}`",
            f"- Commands failed: `{_safe_int(qa_summary.get('commands_failed'), 0)}`",
            f"- Root causes: {', '.join(root_causes) if root_causes else 'none'}",
            "",
            "## Feedback Context",
            f"- Coordinator feedback: {coordinator_note}",
            f"- Execution feedback: {execution_note}",
            "",
...
def _materialize_final_project(
    *,
    job_id: str,
    execution_round: int,
    goal: str,
    ...
    root = Path(os.getcwd()) / "outputs" / job_id / "final_project"
    if root.exists():
        shutil.rmtree(root)
    ...
    for output_index, output in enumerate(outputs):
        agent_id = str(output.get("agent_id") or f"agent-{output_index + 1}")
        task_ids = _string_list(output.get("task_ids"))
        patch_summary = str(output.get("patch_summary") or "No patch summary returned.")
        raw_file_contents = output.get("file_contents")

        file_contents: dict[str, str] = {}
        if isinstance(raw_file_contents, dict):
            for key, value in raw_file_contents.items():
                if isinstance(key, str) and isinstance(value, str):
                    file_contents[key] = value

        changed_files = _string_list(output.get("changed_files"))
        if not changed_files and file_contents:
            changed_files = sorted(file_contents.keys())
        if not changed_files:
            changed_files = [f"src/generated/{agent_id}_round_{execution_round}.py"]

        for file_index, changed_file in enumerate(changed_files):
            rel_path = _safe_output_relative_path(
                changed_file,
                fallback=f"src/generated/{agent_id}_round_{execution_round}_{file_index + 1}.py",
            )
            destination = root / rel_path
            destination.parent.mkdir(parents=True, exist_ok=True)
            file_text = file_contents.get(changed_file) or file_contents.get(rel_path.as_posix())
            if not isinstance(file_text, str) or not file_text.strip():
                file_text = _simulated_file_content(
                    agent_id=agent_id,
            ...
            destination.write_text(file_text, encoding="utf-8")
            written_files.add(rel_path.as_posix())
    ...
    readme_lines = [
        ...
        f"- Job ID: `{job_id}`",
        f"- Execution round: `{execution_round}`",
        f"- Prompt goal: {_truncate_text(goal, limit=500)}",
        f"- Plan excerpt: {_truncate_text(plan, limit=700)}",
        f"- Task assignments: `{assignment_count}`",
        f"- Coder files materialized: `{len(written_files)}`",
        f"- Merge status: `{(merge_result or {}).get('status', 'unknown')}`",
        f"- QA status: `{(qa_result or {}).get('status', 'unknown')}`",
        "",
        "## Files",
    ]
    readme_lines.extend(f"- `{path}`" for path in sorted(written_files))
    (root / "README.md").write_text("\n".join(readme_lines) + "\n", encoding="utf-8")
    written_files.add("README.md")
    return str(root), sorted(written_files)
...
def _persist_execution_outputs(
    *,
    ...
    root = Path(os.getcwd()) / "outputs" / job_id
    ...
    for output_index, output in enumerate(outputs):
        ...
        if not changed_files and file_contents:
            changed_files = sorted(file_contents.keys())
        if not changed_files:
            changed_files = [f"generated/{agent_id}_round_{execution_round}.py"]

        for file_index, changed_file in enumerate(changed_files):
            rel_path = _safe_output_relative_path(
                changed_file,
                fallback=f"generated/{agent_id}_round_{execution_round}_{file_index + 1}.txt",
            )
    ...
    review_rel_path = _safe_output_relative_path(
        f"reviews/round_{execution_round}_review.md",
        fallback=f"reviews/round_{execution_round}_review.md",
    )
    review_destination = files_root / review_rel_path
    review_destination.parent.mkdir(parents=True, exist_ok=True)
    review_destination.write_text(
        _build_round_review_markdown(
            execution_round=execution_round,
            goal=goal,
            plan=plan,
    ...
    written_files.add(review_rel_path.as_posix())

    manifest = {
        "job_id": job_id,
        "execution_round": execution_round,
        "goal": goal,
        "plan_excerpt": _truncate_text(plan, limit=1000),
        "written_files": sorted(written_files),
        "user_agent_outputs": outputs,
        "task_distribution": task_distribution,
        "coordinator_context": coordinator_context,
        "execution_context": execution_context,
        "merge_result": merge_result,
...
async def start_job(req: JobCreateReq):
    ...
    job["simulation_mode"] = _is_simulation_mode(req)
    _jobs[job_id] = job
...
async def get_status(job_id: str):
    ...
    return {
        ...
        "logicalAgentStates": _logical_agent_states(job),
        ...
        "userAgentOutputs": job["user_agent_outputs"],
        "simulationMode": bool(job.get("simulation_mode")),
        "outputPath": job.get("output_path"),
        "writtenFiles": job.get("written_files") or [],
        "finalProjectPath": job.get("final_project_path"),
        "finalProjectFiles": job.get("final_project_files") or [],
        "workflowContext": job["workflow_context"],
        "planningRound": job["planning_round"],
        "coordinationRound": job["coordination_round"],
        "executionRound": job["execution_round"],
    }
        ...
        outputs.append(
            {
                ...
                "file_contents": {
                    f"src/generated/{agent_id}_round_{execution_round}.py": _simulated_file_content(
                        agent_id=agent_id,
                        changed_file=f"src/generated/{agent_id}_round_{execution_round}.py",
                        patch_summary=(
                            "Retry-aware patch generated with QA feedback context."
                            if execution_context
                            else "Initial patch generated from approved task assignments."
            ...
            outputs.append(
                {
                    ...
                    "file_contents": {
                        f"src/generated/coder_{idx + 1}_round_{execution_round}.py": _simulated_file_content(
                            agent_id=f"coder-{idx + 1}",
                            changed_file=f"src/generated/coder_{idx + 1}_round_{execution_round}.py",
                            patch_summary="Fallback output due to missing explicit assignments.",
                            task_ids=[f"task-fallback-{idx + 1}"],
                            execution_round=execution_round,
                            goal=req.goal,
...
async def _run_coding_agents(
    ...
    if _is_simulation_mode(req):
        return _build_user_agent_outputs(req, execution_round, execution_context, task_distribution, plan)
    ...
    async def _run_assignee(agent_id: str, task_list: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        raw_file_contents = result.get("file_contents")
        if isinstance(raw_file_contents, dict):
            normalized_file_contents: dict[str, str] = {}
            for key, value in raw_file_contents.items():
                if isinstance(key, str) and isinstance(value, str):
                    normalized_file_contents[key] = value
            if normalized_file_contents:
                output["file_contents"] = normalized_file_contents
                if not changed_files:
                    changed_files = sorted(normalized_file_contents.keys())
                    output["changed_files"] = changed_files

        code_blob = result.get("code")
        if isinstance(code_blob, str) and code_blob.strip():
            if not changed_files:
                changed_files = [f"src/generated/{agent_id}_round_{execution_round}.py"]
                output["changed_files"] = changed_files
            output.setdefault("file_contents", {})
            if isinstance(output.get("file_contents"), dict):
                output["file_contents"].setdefault(changed_files[0], code_blob)
...
async def _run_pipeline(job_id: str, req: JobCreateReq):
    ...
    -> final user approval (reject sends back to coding/verification)
    """
    job = _jobs[job_id]
    ...
    try:
        while True:
            ...
            while True:
                ...
                _set_agent_result(job, "planner", job["plan"] or "")
            ...
            job["written_files"] = []
            job["final_project_path"] = None
            job["final_project_files"] = []
            ...
            while True:
                ...
                assignment_count = len((job["task_distribution"] or {}).get("assignments") or [])
                _set_agent_result(
                    job,
                    "task_coordinator",
                    (
                        f"# Task Coordinator Result\n\n"
                        f"- Coordination round: **{job['coordination_round']}**\n"
                        f"- Assignments generated: **{assignment_count}**\n"
                        f"- Context reason: {(job['task_distribution'] or {}).get('context_reason') or 'None'}"
                    ),
                )
                ...
                _set_agent_result(
                    ...
                    "conflict_analyst",
                    (
                        f"# Conflict Analysis Result\n\n"
                        f"- Score: **{(job['conflict_report'] or {}).get('overall_conflict_score', 0)}%**\n"
                        f"- Threshold: **{(job['conflict_report'] or {}).get('threshold_percent', 20)}%**\n"
                        f"- Breached: **{'Yes' if (job['conflict_report'] or {}).get('threshold_breached') else 'No'}**\n"
                        f"- Next action: `{(job['conflict_report'] or {}).get('next_action') or 'unknown'}`"
                    ),
                )
                ...
                while True:
                    ...
                    coder_lines = [
                        ...
                        "",
                        f"- Execution round: **{job['execution_round']}**",
                    ]
                    for output in job["user_agent_outputs"]:
                        agent_id = output.get("agent_id", "unknown-agent")
                        changed_files = ", ".join(_string_list(output.get("changed_files"))) or "None"
                        patch_summary = output.get("patch_summary") or "No patch summary returned."
                        coder_lines.extend(
                            [
                                f"## {agent_id}",
                    ...
                    _set_agent_result(job, "user_agents", "\n".join(coder_lines))
                    output_path, written_files = _persist_execution_outputs(
                        job_id=job_id,
                        execution_round=job["execution_round"],
                        goal=job.get("goal") or req.goal,
                        plan=job.get("plan") or "",
                        task_distribution=job.get("task_distribution"),
                        coordinator_context=job["workflow_context"].get("coordinator_feedback"),
                        execution_context=job["workflow_context"].get("execution_feedback"),
                        outputs=job["user_agent_outputs"],
                        merge_result=job.get("merge_result"),
                        qa_result=job.get("qa_result"),
                    )
                    job["output_path"] = output_path
                    job["written_files"] = written_files
                    final_project_path, final_project_files = _materialize_final_project(
                        job_id=job_id,
                        execution_round=job["execution_round"],
                        goal=job.get("goal") or req.goal,
                        plan=job.get("plan") or "",
                    ...
                    job["final_project_path"] = final_project_path
                    job["final_project_files"] = final_project_files
                    _set_agent_state(job, "user_agents", "done")
                    _log(
                        job,
                        f"✓ User agent outputs collected. Artifacts: {output_path} ({len(written_files)} files). "
                        f"Materialized project: {final_project_path} ({len(final_project_files)} files).",
                    )
                    ...
                    merge_summary = (job["merge_result"] or {}).get("summary") or {}
                    _set_agent_result(
                        job,
                        "merge_agent",
                        (
                            f"# Merge Agent Result\n\n"
                            f"- Status: **{(job['merge_result'] or {}).get('status', 'unknown')}**\n"
                            f"- Mergeable: **{'Yes' if (job['merge_result'] or {}).get('mergeable') else 'No'}**\n"
                            f"- Outputs merged: **{_safe_int(merge_summary.get('total_outputs'), 0)}**\n"
                            f"- Conflicts detected: **{_safe_int(merge_summary.get('conflicts_detected'), 0)}**\n"
                            f"- Next action: `{(job['merge_result'] or {}).get('next_action') or 'unknown'}`"
                        ),
                    )
                    output_path, written_files = _persist_execution_outputs(
                        job_id=job_id,
                    ...
                    qa_summary = (job["qa_result"] or {}).get("summary") or {}
                    qa_failures = ((job["qa_result"] or {}).get("failure_report") or {}).get("root_causes") or []
                    _set_agent_result(
                        job,
                        "qa_agent",
                        (
                            f"# QA Agent Result\n\n"
                            f"- Status: **{(job['qa_result'] or {}).get('status', 'unknown')}**\n"
                            f"- QA passed: **{'Yes' if (job['qa_result'] or {}).get('qa_passed') else 'No'}**\n"
                            f"- Commands run: **{_safe_int(qa_summary.get('commands_run'), 0)}**\n"
                            f"- Commands failed: **{_safe_int(qa_summary.get('commands_failed'), 0)}**\n"
                            f"- Primary failure: {(qa_failures[0] if qa_failures else 'None')}"
                        ),
                    )
                    output_path, written_files = _persist_execution_outputs(
                        job_id=job_id,
                    ...
                    rejection_feedback = (
                        ...
                        or "User requested revisions before acceptance."
                    )
                    job["workflow_context"]["execution_feedback"] = {
                        "source": "final_rejection",
                        "reason": rejection_feedback,
                        "failure_report": {
                            "root_causes": [rejection_feedback],
                            "failed_commands": [],
                        },
                    }
                    _log(job, "❌ User rejected final output. Returning to coding with user feedback context.")
                    continue
